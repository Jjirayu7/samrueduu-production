/*
  Warnings:

  - You are about to drop the column `cost123` on the `BillSaleDetail` table. All the data in the column will be lost.
  - Added the required column `cost` to the `BillSaleDetail` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "BillSaleDetail" DROP COLUMN "cost123",
ADD COLUMN     "cost" INTEGER NOT NULL;
